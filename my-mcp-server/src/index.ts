...
interface McpTools {
	[key: string]: {
		description: string;
		inputSchema: any;
		handler: (args: any, apiToken: string) => Promise<any>;
	};
}
...
function createMcpTools(apiToken: string): McpTools {
	return {
		...
		list_functions: {
			...
			handler: async () => {
				try {
					const client = new MicroFnApiClient(apiToken);
		...
		updateFunctionCode: {
			...
			inputSchema: {
				...
				required: ["workspaceId", "functionName", "code"],
			},
			handler: async (args) => {
				try {
		...
		if (url.pathname === "/mcp") {
			...
			try {
				...
				if (body.method === "tools/call") {
					...
					try {
						const result = await tools[toolName].handler(toolArgs, apiToken);
						return new Response(
							JSON.stringify({
								jsonrpc: "2.0",
								result,
