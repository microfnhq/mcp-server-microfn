...
function createMcpTools(apiToken: string): McpTools {
  return {
    ...
    packageManager: {
      description: "Manage packages for a function (install, update, remove, list)",
      inputSchema: {
        type: "object",
        properties: {
          workspaceId: { type: "string" },
          functionName: { type: "string" },
          action: { type: "string", enum: ["install", "update", "remove", "list", "updateLayer"] },
      ...
      handler: async (args) => {
        try {
          const result = await handlePackageManager(
            apiToken,
            args,
            {},
            {} as ExecutionContext,
