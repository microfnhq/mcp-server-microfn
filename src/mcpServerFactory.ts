...
export async function createMcpServer(
    ...
    server.tool(
        ...
        async ({ testParam }) => {
            console.log(
                "[mcpServerFactory] testParams called with testParam:",
                testParam,
            );
    ...
    const listFunctionsTool = server.tool(
        ...
        async (extra: any) => {
            console.log(
                "[mcpServerFactory] listFunctions called with extra:",
                extra,
            );
            ...
            try {
                ...
                console.error(
                    "[mcpServerFactory] listFunctions error:",
                    error.message,
                );
                return {
                    content: [
    ...
    server.tool(
        ...
        "Create a new MicroFn workspace/function with TypeScript code. The code must export either: 1) A 'main' function as the entry point, OR 2) Any named function (which will be auto-wrapped with a main function). Examples: 'export async function main() { return \"hello\" }', 'export async function sayHello(name) { return \"hello \" + name }' (auto-wrapped), 'export async function main() { const btc = await fetch(\"...\"); return btc.json() }'. IMPORTANT: Import @microfn/* modules as defaults: 'import kv from \"@microfn/kv\"' NOT 'import { kv } from \"@microfn/kv\"'. Common patterns: Use @microfn/secret for env vars, @microfn/kv for persistent storage (e.g., counters, cache), fetch() for HTTP requests. If unsure about syntax, use the 'rewriteFunction' tool to transform your code. Input: 'name' (string) - function name, 'code' (string) - TypeScript code. Output: Returns the created workspace object with id, name, and metadata.",
        {
            name: z
                .string()
                .describe("The name for the new function/workspace"),
            code: z
                .string()
                .describe(
    ...
    const getFunctionCodeTool = server.tool(
        ...
        async ({ functionName }) => {
            ...
            try {
                ...
                console.error(
                    "[mcpServerFactory] getFunctionCode error:",
                    error.message,
                );
                return {
                    content: [
    ...
    server.tool(
        ...
        async ({ functionName }) => {
            console.log(
                "[mcpServerFactory] listPackages called with functionName:",
                functionName,
            );
    ...
    server.tool(
        ...
        "Rename a function/workspace",
        {
            ...
            newName: z
                .string()
                .describe(
                    "The new name for the function (just the name, not username/)",
                ),
        },
    ...
    server.tool(
        ...
        async ({ prompt }) => {
            console.log(
                "[mcpServerFactory] generateFunction called with prompt:",
                prompt,
            );
    ...
    server.tool(
        ...
        async ({ code }) => {
            console.log(
                "[mcpServerFactory] rewriteFunction called with code length:",
                code.length,
            );
    ...
    // Dynamic tool registration - fetch and register user functions with mcpToolEnabled
    // This happens synchronously during server creation to ensure tools are available for tools/list
        ...
        try {
            ...
            workspaces.forEach((workspace) => {
                if (workspace.mcpToolEnabled) {
                    ...
                    server.tool(
                        toolName,
                        `Execute the ${workspace.name} microfn function`,
                        {
                            input: z
                                .string()
                                .describe(
                                    "The input to pass to the microfn function",
                                ),
                    ...
                    registeredCount++;
                    console.log(
                        `[mcpServerFactory] ✓ Registered dynamic MCP tool: ${toolName}`,
                    );
                }
            });
            ...
            if (registeredCount > 0) {
                console.log(
                    `[mcpServerFactory] Successfully registered ${registeredCount} dynamic tool(s)`,
                );
            ...
            console.error(
                "[mcpServerFactory] Failed to register dynamic tools:",
                error,
            );
    ...
    // Execute the dynamic registration but don't await it to avoid blocking
    // The tools will be registered before any subsequent requests
