...
export class UserDataCache extends DurableObject {
	...
	async getFunctions(): Promise<Workspace[] | null> {
		const cached = await this.ctx.storage.get<CachedData<Workspace[]>>("functions");

		if (cached && Date.now() - cached.timestamp < UserDataCache.CACHE_DURATION_MS) {
			console.log("[UserDataCache] Returning cached functions:", cached.data.length);
			return cached.data;
		}
