...
export class MicroFnApiClient {
    ...
    private validateToken(): void {
        ...
        if (
            this.apiToken.startsWith("mfn_") ||
            this.apiToken.startsWith("mfp_")
        ) {
            return;
        }
        ...
        try {
            ...
            if (currentTime >= tokenExp) {
                ...
                console.error(
                    "[MicroFnApiClient] Auth0 ID token expired at:",
                    expiredAt,
                );
                throw new Error(
                    `Auth0 ID token expired at ${expiredAt}. Please re-authenticate.`,
                );
            }
        } catch (error) {
    ...
    async listPackages(
        ...
        if (!res.ok)
            throw new Error(`Failed to list packages: ${res.statusText}`);
    ...
    async installPackage(
        ...
        const res = await fetch(
            `${this.baseUrl}/workspaces/${username}/${functionName}/packages`,
            {
                method: "POST",
                headers: this.getHeaders(),
        ...
        if (!res.ok)
            throw new Error(`Failed to install package: ${res.statusText}`);
        const data = (await res.json()) as {
    ...
    async removePackage(
        ...
        if (!res.ok)
            throw new Error(`Failed to remove package: ${res.statusText}`);
    }
    ...
    async updatePackageLayer(
        ...
        if (!res.ok)
            throw new Error(
                `Failed to update package layer: ${res.statusText}`,
            );
    ...
    async updateWorkspace(
        ...
        const res = await fetch(
            `${this.baseUrl}/workspaces/${username}/${functionName}/code`,
            {
                method: "POST",
                headers: this.getHeaders(),
    ...
    async renameWorkspace(
        ...
        const res = await fetch(
            `${this.baseUrl}/workspaces/${username}/${functionName}`,
            {
                method: "PATCH",
                headers: this.getHeaders(),
    ...
    async updateFunctionCode(
        ...
        if (!res.ok)
            throw new Error(
                `Failed to update function code: ${res.statusText}`,
            );
        return await res.json();
    }
    ...
    async executeFunction(
        ...
        console.log("[MicroFnApiClient] Response:", res.status, res.statusText);
        if (!res.ok)
            throw new Error(`Failed to execute function: ${res.statusText}`);
        ...
        try {
            ...
            console.log(
                "[MicroFnApiClient] Execution result (text):",
                textResult,
            );
    ...
    async getLatestDeployment(
        ...
        if (!res.ok)
            throw new Error(
                `Failed to get latest deployment: ${res.statusText}`,
            );
    ...
    async listSecrets(
        ...
        const res = await fetch(
            `${this.baseUrl}/workspaces/${username}/${functionName}/secrets`,
            {
                method: "GET",
                headers: this.getHeaders(),
            },
    ...
    async createSecret(
        ...
        if (!res.ok)
            throw new Error(`Failed to create secret: ${res.statusText}`);
        const data = (await res.json()) as { secrets?: Secret[] };
        return data.secrets || [];
    }
    ...
    async deleteSecret(
        ...
        if (!res.ok)
            throw new Error(`Failed to delete secret: ${res.statusText}`);
    }
    ...
    async rewriteFunction(
        ...
        console.log(
            "[MicroFnApiClient] Rewritten code length:",
            data.code?.length || 0,
        );
        return data;
    }
