...
class SimpleJSONRPCTransport {
    ...
    async close(): Promise<void> {
        if (this.connectionState === "connected") {
            ...
            console.log(
                `[Transport] Connection closed after ${this.getConnectionDuration()}ms`,
            );
        }
        this.onclose?.();
    }
...
export class StreamableHTTPHandler {
    ...
    async handle(
        ...
        try {
                ...
                return new Response(
                    JSON.stringify({
                        ...
                        error: {
                            code: 401,
                            message:
                                "No Auth0 ID token available. Please re-authenticate.",
                        },
                        id: null,
                    }),
                    {
            ...
            console.log(
                "[StreamableHTTP] ID token prefix:",
            ...
            console.log(
                "[StreamableHTTP] Full request body:",
                JSON.stringify(body),
            );
...
export function createStreamableHTTPHandler() {
    return {
        fetch: async (
            ...
            console.log(
                "[StreamableHTTP] createStreamableHTTPHandler called:",
                {
                    hasProps: !!userProps,
            ...
            if (!userProps) {
                console.error(
                    "[StreamableHTTP] No user props provided by OAuth provider",
                );
                ...
                return new Response(
                    JSON.stringify({
                        ...
                        error: {
                            code: -32603,
                            message:
                                "Internal error: OAuth provider did not provide user context",
                        },
                        id: null,
                    }),
                    {
