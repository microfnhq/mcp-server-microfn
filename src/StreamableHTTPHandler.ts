...
export class StreamableHTTPHandler {
  ...
  async handle(request: Request, executionContext: ExecutionContext): Promise<Response> {
    try {
      ...
      if (!apiToken) {
        console.error('[StreamableHTTP] No MicroFn PAT available for user:', this.userProps.claims?.email);
      ...
      console.log('[StreamableHTTP] Handling request for user:', this.userProps.claims?.email);
      ...
      if (request.method !== 'POST') {
        return new Response('Method not allowed. Use GET to establish session or POST with JSON-RPC payload.', { 
          status: 405,
          headers: {
            'Allow': 'GET, POST',
          },
...
export function createStreamableHTTPHandler() {
  return {
    fetch: async (request: Request, env: Env, ctx: ExecutionContext & { props?: UserProps }) => {
